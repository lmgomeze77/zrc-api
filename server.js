...
app.use(cors({ origin: ["https://zenithrisecapital.com","https://www.zenithrisecapital.com", process.env.FRONTEND_URL || "http://localhost:5173"], methods: ["POST","GET"] }));
...
app.post("/api/submit", async (req, res) => {
  try {
    ...
    const subjectMap = { teaser:`ðŸ¢ Teaser Request: ${context||"Investment Opportunity"}`, advisory:`ðŸ’¼ Advisory Inquiry from ${company||name}`, enroll:`ðŸŽ“ Enrollment Request: ${context||"Course"}`, apply:`ðŸ”‘ Inner Circle Application: ${name}`, contact:`ðŸ“© Contact from ${name} â€” ${company||"N/A"}`, register:`ðŸ‘¤ New Registration: ${name} (${email})` };
    const subject = subjectMap[type] || `ðŸ“© ZRC Form: ${type} from ${name}`;
...
app.post("/api/register", async (req, res) => {
  try {
    ...
    await resend.emails.send({ from: FROM_EMAIL, to: [NOTIFY_EMAIL], subject: `ðŸ‘¤ New Platform Registration: ${name}`, html: buildEmailHTML({ type: "registration", data: { name, email, company, role, interest } }) });
    await resend.emails.send({ from: FROM_EMAIL, to: [email], subject: "Welcome to Zenith Rise Capital", html: `<body style="background:#09090B;color:#FAFAFA;font-family:'Helvetica Neue',sans-serif;padding:40px;"><h2>Welcome, ${name}</h2><p>You now have access to the ZRC platform. Visit <a href="https://zenithrisecapital.com" style="color:#D4A853;">zenithrisecapital.com</a></p></body>` });
...
app.listen(PORT, () => { console.log(`\n  â—ˆ ZRC API running on port ${PORT}\n  Notify: ${NOTIFY_EMAIL}\n`); });